...
1. Build your Vite app (`npm run build`) to create the static HTML/CSS/JS/etc. website files. (You cannot use the Vite development server for production.)
...
1. Copy all contents of the `dist/` folder (generated by vite build) to you node application's `public/` folder on the server.
   - you do the vite build locally and copy the files to server using e.g. scp or FileZilla or build directly on the deployment server using a cloned git repository     
